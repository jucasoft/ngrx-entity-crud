...
function verdict(info: StoreInfo, lazySections: Set<string>): string {
  ...
  if (info.usedByShell) {
    return 'keep eager (used by the shell)';
  }
  if (info.sections.length === 0) {
    return 'orphan? (no section uses it)';
  }
  if (info.sections.length > 1) {
    return `multi-section (${info.sections.length}) -> consider a shared module`;
  }
  if (!lazySections.has(info.sections[0])) {
    return 'section not lazy-routed -> little benefit from lazy';
  }
...
export function lazyReport(options: LazyReport): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    const candidates = rows.filter((s) => verdict(s, lazySections) === 'lazy candidate');

    const md: string[] = [];
    md.push('# Lazy store report', '');
    md.push('Generated by `ngrx-entity-crud:lazy-report`.', '');
    md.push(
      `Analyzed paths: store=\`${paths.pathStore}\`, views=\`${paths.pathView}\`, app=\`${paths.pathApp}\`.`,
      ''
    );
    md.push('## Stores -> sections', '');
    md.push('| store | clazz | type | sections | n | lazy route | shell | verdict |');
    md.push('|---|---|---|---|---|---|---|---|');
    rows.forEach((s) => {
      const lazyRoute =
        s.sections.length === 0
          ...
          : s.sections.every((x) => lazySections.has(x))
            ...
            : 'no/partial';
      md.push(
        `| ${s.name} | ${s.clazz} | ${s.type} | ${s.sections.join(', ') || '-'} | ${s.sections.length} | ${lazyRoute} | ${s.usedByShell ? 'yes' : 'no'} | ${verdict(s, lazySections)} |`
      );
    });
    md.push('', '## Lazy candidates', '');
    if (candidates.length === 0) {
      md.push('_No lazy candidates found._');
    } else {
      candidates.forEach((s) => {
        const type = s.type === 'unknown' ? 'CRUD-PLURAL' : s.type;
        md.push(`- **${s.clazz}** (\`${s.name}\`), used by \`${s.sections[0]}\`:`);
        md.push(
          `  - \`ng generate ngrx-entity-crud:store --clazz=${s.clazz} --type=${type} --registration=lazy\``
        );
        md.push(`  - then import \`${s.clazz}StoreModule\` in the view's \`${s.sections[0]}.module.ts\` module.`);
      });
    }
    if (storage) {
      md.push('', '## Storage', '');
      if (storage.providers.length) {
        md.push(
          `Persistence providers detected in package.json: ${storage.providers
            .map((p) => '`' + p + '`')
            .join(', ')}.`
        );
      } else {
        md.push('_No known persistence providers detected in package.json._');
      }
      md.push('The dashboard still introspects IndexedDB/localStorage agnostically at runtime.');
    }
    md.push('', '## Notes', '');
    md.push('- Static analysis based on naming conventions (`XxxStoreActions/Selectors/State/Module`).');
    md.push('- Manually check for hidden couplings (effects/selectors combining multiple stores).');
    md.push('- Stores used by the shell (`core/`, `main/components`, `app.component`) stay eager.');
    ...
    const jsonReport: Record<string, unknown> = {
      ...
      stores: rows.map((s) => {
        ...
        return {
          ...
          isLazyCandidate: v === 'lazy candidate',
        };
      }),
    };
    if (storage) {
    ...
    context.logger.info(
      `Stores: ${rows.length} | sections: ${sections.length} | lazy candidates: ${candidates.length}`
    );
