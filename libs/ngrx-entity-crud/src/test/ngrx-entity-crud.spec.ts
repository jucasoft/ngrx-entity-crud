...
describe('NgRx Entity CRUD', () => {
  ...
  const expectSuccessState = (
    resultState: State,
    baseState: State,
    additionalProps: Partial<State> = {}
  ) => {
    expect(resultState).toEqual({
      ...baseState,
      isLoaded: true,
  ...
  describe('Reducers - Request Actions', () => {
    describe('Create Operations', () => {
      ...
      it('should handle CreateManyRequest', () => {
        ...
        const result = featureReducer(
          state,
          actions.CreateManyRequest(payload)
        );
        expectLoadingState(result, state);
      });
    });
    ...
    describe('Delete Operations', () => {
      ...
      it('should handle DeleteManyRequest', () => {
        ...
        const result = featureReducer(
          state,
          actions.DeleteManyRequest(payload)
        );
        expectLoadingState(result, state);
      });
    });
  ...
  describe('Reducers - Success Actions', () => {
    describe('Search Success with Different Modes', () => {
      const newItems = createPizzas(4).map((item, i) => ({
        ...item,
        id: i + 4,
        name: String.fromCharCode(101 + i),
      }));

      it('should handle SearchSuccess with REFRESH mode', () => {
        const lastCriteria = actions.SearchRequest({
          mode: 'REFRESH',
          queryParams: undefined,
          path: undefined,
        });
        ...
        const result = featureReducer(
          { ...state, lastCriteria },
          actions.SearchSuccess({ items: newItems, request: lastCriteria })
        );
        expect(result).toEqual(expectedState);
      });
    ...
    describe('CRUD Success Operations', () => {
      ...
      it('should handle DeleteManySuccess', () => {
        ...
        const result = featureReducer(
          state,
          actions.DeleteManySuccess(payload)
        );
        expect(result).toEqual(expectedState);
      });
    });
  ...
  describe('Reducers - Selection Actions', () => {
    ...
    it('should handle AddManySelected and RemoveManySelected', () => {
      ...
      let entitiesSelected = toDictionary(
        [
          { id: 0, name: 'a' },
          { id: 1, name: 'b' },
          { id: 2, name: 'c' },
      ...
      // Remove selection
      result = featureReducer(
        result,
        actions.RemoveManySelected({ ids: ['0'] })
      );
      entitiesSelected = toDictionary(
        [
          { id: 1, name: 'b' },
          { id: 2, name: 'c' },
        ],
    ...
    it('should handle RemoveAllSelected', () => {
      const entitiesSelected = toDictionary(
        [
          { id: 0, name: 'a' },
          { id: 1, name: 'b' },
        ],
  ...
  describe('Reducers - Failure Actions', () => {
    ...
    const failureTests = [
      {
        name: 'SearchFailure',
        action: (error) => actions.SearchFailure({ error }),
      },
      {
        name: 'DeleteFailure',
        action: (error) => actions.DeleteFailure({ error }),
      },
      {
        name: 'DeleteManyFailure',
        action: (error) => actions.DeleteManyFailure({ error }),
      },
      {
        name: 'CreateFailure',
        action: (error) => actions.CreateFailure({ error }),
      },
      {
        name: 'CreateManyFailure',
        action: (error) => actions.CreateManyFailure({ error }),
      },
      {
        name: 'EditFailure',
        action: (error) => actions.EditFailure({ error }),
      },
      {
        name: 'EditManyFailure',
        action: (error) => actions.EditManyFailure({ error }),
      },
      {
        name: 'SelectFailure',
        action: (error) => actions.SelectFailure({ error }),
      },
    ];
  ...
  describe('Edge Cases', () => {
    it('should handle empty state', () => {
      ...
      const emptyFeatureReducer = adapter.createCrudReducer(
        emptyState,
        actions
      );
    ...
    it('should handle multiple consecutive selections', () => {
      let result = featureReducer(
        state,
        actions.SelectItem({ item: { id: 0, name: 'a' } })
      );
      result = featureReducer(
        result,
        actions.SelectItem({ item: { id: 1, name: 'b' } })
      );
